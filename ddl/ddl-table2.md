# Creating and Managing Tables {#topic_jtv_5vy_sp}
...
-   Any table constraints to limit the data that a column or table can contain. See [Setting Table Constraints](#).
-   The distribution policy of the table, which determines how HAWQ divides data is across the segments. See [Choosing the Table Distribution Policy](#).
-   The way the table is stored on disk.
-   The table partitioning strategy for large tables, which specifies how the data should be divided. See [Creating and Managing Databases](ddl-database.md).

### Choosing Column Data Types {#topic27}
...
### Setting Table Constraints {#topic28}
...
```
=> CREATE TABLE products 
       ( product_no integer **NOT NUL**L,
         name text **NOT NULL**,
         price numeric );

```

#### Foreign Keys {#topic33}
...
### Choosing the Table Distribution Policy {#topic34}
...
| |Syntax|
|--|------|
|INHERITS|`create table new_table inherit (origintable) [with(bucketnum=x)] [distributed by col]`|
...
#### Declaring Distribution Keys {#topic_kjg_tqm_gv}
...
#### Performance Tuning {#topic_wff_mqm_gv}
